      ...
      <div style=" max-height:500px;overflow-y:scroll ; ">
       <table class="table table-striped"   >
        ...
        <?php  $sqlaccounts ="SELECT Classifications from equipment ";
         ...
         <tbody>
           <?php $query = "SELECT MAX($name) as max from equipment";
           ...
           for($i = 1; $i<=$max; $i++){ ?>
                <tr>
                    <td><?php echo $i ?></td>
                <?php 
           $query = "SELECT e.$name ,i.Classifications from equipment e join inventory i on i.No = e.No"  ;
           $result = $sqlconnect->query($query);
           while(  $rows = $result->fetch_assoc()){
                    $class = $rows['Classifications'];
                       if($rows["$name"] == '0'){ ?>
                          <td>N/A</td>
                      <?php } else { 
                        $query = "select i.Code, e.$name ,concat(i.$name,'-',$i) as $name from inventory i  join  equipment e  on i.No = e.No  
                        where  e.$name != 0 and  i.Classifications= '$class'";
